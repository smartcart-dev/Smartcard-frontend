...
export class StoreMapComponent implements AfterViewInit, OnDestroy {
  ...
  setTransformMode(mode: 'translate' | 'rotate' | 'scale'): void {
    ...
    if (this.transformControl) {
      ...
      // Show all axes for all modes — allow full freedom of movement
  ...
  spawnObject(type: string): void {
     ...
     if (newObject) {
        ...
        // Select immediately — no setTimeout needed, stays in Angular zone
  ...
  private initThreeJs(): void {
    ...
    // Orbit Controls — no angle restrictions for full 360° rotation
  ...
  private animate(): void {
    ...
    // No scene rotation — OrbitControls.autoRotate handles the subtle camera movement
